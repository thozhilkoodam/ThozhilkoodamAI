...
import { Eye, EyeOff, Phone, Loader2, Users } from 'lucide-react'
import { BrandLogo } from '@/components/brand-logo'
import { api } from '@/lib/api-client'
import toast from 'react-hot-toast'

export default function HrConsultantLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const result = await api.hrConsultants.login(form.email, form.password)
      if (!result) throw new Error('Invalid credentials')
      localStorage.setItem('access_token', result.accessToken)
      localStorage.setItem('thozhil_hr_consultant_user', JSON.stringify(result.consultant))
      if (rememberMe) localStorage.setItem('thozhil_hr_consultant_remember', form.email)
      else localStorage.removeItem('thozhil_hr_consultant_remember')
  ...
  const handleGoogleLogin = () => {
    router.push('/business-operations/hr-consultant/google-login')
  }
  ...
  const handleOtpLogin = async () => {
    ...
    if (!otpSent) {
      const response = await api.hrConsultants.sendOtp(phone)
      setOtpSent(true)
    ...
    const result = await api.hrConsultants.verifyOtp(phone, otp)
    if (!result) {
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="w-full max-w-md border-0 shadow-xl">
          <CardHeader className="text-center">
            ...
            <CardTitle className="mt-4 text-2xl font-bold">HR Recruiter Login</CardTitle>
            ...
            {!showOtp ? (
              <>
                ...
                <div className="mt-6 text-center text-sm text-muted-foreground">
                  New HR Recruiter?{' '}
                  <Link href="/business-operations/hr-consultant/signup" className="text-purple-600 hover:underline font-medium">
                    Sign up
                  </Link>
                </div>
              </>
