...
export default function RejectedJobsPage() {
  return <JobListPage status="rejected" title="Rejected Jobs" />
}
...
function JobListPage({ status, title }: { status: string; title?: string }) {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">{title || 'Rejected Jobs'}</h1><p className="text-sm text-muted-foreground">View all {status} job postings.</p></div>
      ...
      <Card><CardContent className="p-0">
        ...
        : jobs.length === 0 ? <div className="py-20 text-center text-muted-foreground"><p className="text-lg">No {status} jobs</p></div>
        : <div className="overflow-x-auto"><Table>
          <TableHeader><TableRow className="bg-muted/50">
            <TableHead>Title</TableHead><TableHead>Company</TableHead><TableHead>Location</TableHead><TableHead>Type</TableHead><TableHead>Experience</TableHead><TableHead>Salary</TableHead><TableHead>Status</TableHead><TableHead>Reason</TableHead><TableHead>Created</TableHead>
          ...
          <TableBody>{jobs.map((job) => (
            <TableRow key={job.id} className="hover:bg-muted/50">
              ...
              <TableCell className="text-xs text-muted-foreground max-w-[200px] truncate">{job.rejectedReason || '-'}</TableCell>
