...
export default function PublishedJobsPage() {
  return <JobListPage status="published" title="Published Jobs" />
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">{title || 'Published Jobs'}</h1>
          <p className="text-sm text-muted-foreground">View all {status} job postings.</p>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-20 text-center text-muted-foreground">
              <p className="text-lg">No {status} jobs</p>
            </div>
          ) : (
                  ...
                  {jobs.map((job) => (
                    <TableRow key={job.id} className="hover:bg-muted/50">
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => window.open(`/admin/jobs/pending`, '_blank')}>
